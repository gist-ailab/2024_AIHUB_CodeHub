...
def capture_rgbd(args):
    ...
    # check if the save directory exists (scene_id)
    if not os.path.exists(os.path.join(args.save_dir, 'rgb', f'group_{group_id:06d}/scene_{scene_id:06d}')):
        os.makedirs(os.path.join(args.save_dir, 'rgb', f'group_{group_id:06d}/scene_{scene_id:06d}'))
    if not os.path.exists(os.path.join(args.save_dir, 'depth', f'group_{group_id:06d}/scene_{scene_id:06d}')):
        os.makedirs(os.path.join(args.save_dir, 'depth', f'group_{group_id:06d}/scene_{scene_id:06d}'))

    rgb_save_dir = os.path.join(args.save_dir, 'rgb')
    depth_save_dir = os.path.join(args.save_dir, 'depth')

    cv2_window_name = 'rgbd - group_id: ' + str(group_id) + ' scene_id: '+ str(scene_id) + ' cut_id: '+ str(cut_id) + '  ' + 'C: Save & Next Cut / S: Next Scene / G: Next Group / Q: Quit'
    ...
    while True:
        ...
        print('group {} / scene {} / cut {}'.format(group_id, scene_id, cut_id))
        ...
        if key == ord('c'):
            ...
            # for img0
            cv2.imwrite(os.path.join(rgb_save_dir, f'group_{group_id:06d}/scene_{scene_id:06d}/{cut_id:06d}.png'), color_image0)
            np.save(os.path.join(depth_save_dir, f'group_{group_id:06d}/scene_{scene_id:06d}/{cut_id:06d}.npy'), depth_image0)
            print(f'{group_id:06d}_{scene_id:06d}_{cut_id:06d} RGBD image saved')
            cut_id += 1
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--scene_id', type=int, default=0)
